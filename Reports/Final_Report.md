...
The modern job market presents unprecedented challenges for job seekers. We observed that professionals often spend an average of 11 hours per week on job searching activities, with hundreds of applications required to secure a single interview. We developed CareerAutomate to solve this problem, offering an intelligent platform that automates repetitive tasks while providing AI-powered insights.

Our platform connects with popular services like GitHub, LinkedIn, Naukri, and Indeed. It automatically syncs user projects, generates professional descriptions using AI, builds tailored resumes, and automates the application process.

## 1.2 Problem Statement

When we started this project, we identified several critical challenges job seekers face:

**Time-Consuming Manual Processes:** Searching for jobs across multiple portals and customizing resumes takes significant time. We found that job seekers apply to 50-100 positions before receiving an offer.

**Inconsistent Personal Branding:** Maintaining updated profiles across platforms is hard. We noticed GitHub projects often lack professional descriptions suitable for resumes.

**Lack of Insights:** Job seekers lack data-driven insights. They don't know which resume versions work best or where they have skill gaps.

**Certificate Management:** There is no centralized system for professionals to manage and verify their accumulated certifications.

**Missed Opportunities:** Jobs are filled quickly. Manual application processes are often too slow to match the speed of the market.
...
| :--- | :--- | :--- | :--- | :--- |
| **Auth** | `/auth/token` | POST | Verify JWT generation on login | PASS |
| **Auth** | `/auth/register` | POST | Account creation with validation | PASS |
| **Github** | `/projects/sync` | POST | Trigger repo sync from GitHub | PASS |
| **Github** | `/projects/{id}` | GET | Retrieve project details | PASS |
| **AI** | `/projects/describe` | POST | specific repo description generation | PASS |
| **Resume** | `/resumes/build` | POST | Generate PDF resume | PASS |
| **Jobs** | `/jobs/fetch` | GET | Fetch jobs from external portals | PASS |
...
1.  **User Onboarding:**
    *   Verified that a new user can sign up, complete their profile, and that data persists in Supabase.
    *   Confirmed that the authentication state is correctly maintained across page reloads.

2.  **GitHub Connection:**
    *   Tested the OAuth flow: Clicking "Connect GitHub" -> Redirect to GitHub -> Authorization -> Redirect back to App.
    *   Verified that the access token is securely stored and used for subsequent requests.

3.  **AI Generation:**
    *   Triggered the "Generate AI Description" feature for a repository.
    *   Confirmed that the backend retrieves the README, sends it to Gemini AI, and saves the returned summary to the database.
...
*   **Step:** Register -> Onboard -> Connect GitHub -> Sync Repos.
*   **Result:** The user was able to complete the flow without errors. Repositories appeared in the dashboard within seconds.

**Scenario 2: Job Application Flow**
*   **Step:** Search Jobs -> Select Job -> Apply with Resume.
*   **Result:** The application status tracked correctly in the "Applications" tab.

**Scenario 3: Certificate Verification (Admin)**
...
*   **Average API Response:** ~150-300ms for standard CRUD operations.
...
*   **Route Protection:** Verified that trying to access `/dashboard` without being logged in redirects to the login page.
*   **Data Isolation:** Confirmed that a user cannot see another user's repositories or resumes by manipulating IDs in the URL.
